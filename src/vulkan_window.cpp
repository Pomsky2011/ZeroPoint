...
VulkanWindow::VulkanWindow(int scale)
    : window(nullptr), scale(scale), quit(false)
    ...
    , commandPool(VK_NULL_HANDLE), currentFrame(0)
    ...
    , stagingBufferMapped(nullptr), descriptorPool(VK_NULL_HANDLE)
{
}
...
bool VulkanWindow::init() {
    try {
        ...
        window = SDL_CreateWindow("ZeroPoint Vulkan",
            SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
            FB_WIDTH * scale, FULL_HEIGHT * scale,
...
void VulkanWindow::render(const Display& display) {
    ...
    // Copy framebuffer data to staging buffer (PERSISTENT MAPPED - no allocation overhead!)
    for (int y = 0; y < FULL_HEIGHT; y++) {
        display.getScanlineSDL(y, static_cast<uint32_t*>(stagingBufferMapped) + y * FB_WIDTH);
    }

    // Record command buffer with texture upload + render in one submit
    ...
    barrier.oldLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;  // From previous frame
    ...
    vkCmdPipelineBarrier(commandBuffers[currentFrame], VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
    ...
    region.imageExtent = {FB_WIDTH, FULL_HEIGHT, 1};

    vkCmdCopyBufferToImage(commandBuffers[currentFrame], stagingBuffer, textureImage,
    ...
    vkCmdDraw(commandBuffers[currentFrame], 6, 1, 0, 0);  // 6 vertices for fullscreen quad
...
void VulkanWindow::createInstance() {
    ...
    appInfo.pApplicationName = "ZeroPoint";
...
void VulkanWindow::createGraphicsPipeline() {
    ...
    vkCreateShaderModule(device, &createInfo, nullptr, &vertShaderModule);

    createInfo.codeSize = fragShaderCode.size();
    createInfo.pCode = reinterpret_cast<const uint32_t*>(fragShaderCode.data());
    vkCreateShaderModule(device, &createInfo, nullptr, &fragShaderModule);
...
void VulkanWindow::createTextureImage() {
    createImage(FB_WIDTH, FULL_HEIGHT, VK_FORMAT_R8G8B8A8_UNORM,
...
void VulkanWindow::createStagingBuffer() {
    VkDeviceSize bufferSize = FB_WIDTH * FULL_HEIGHT * 4;

    createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT,
        VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
        stagingBuffer, stagingBufferMemory);

    // PERSISTENT MAPPING - do this ONCE, not per frame!
...
VkPresentModeKHR VulkanWindow::chooseSwapPresentMode(const std::vector<VkPresentModeKHR>& availablePresentModes) {
    // MAILBOX for lowest latency with no tearing
    for (const auto& availablePresentMode : availablePresentModes) {
        if (availablePresentMode == VK_PRESENT_MODE_MAILBOX_KHR) {
            return availablePresentMode;
        }
    }
    // IMMEDIATE for maximum speed (may tear)
    for (const auto& availablePresentMode : availablePresentModes) {
